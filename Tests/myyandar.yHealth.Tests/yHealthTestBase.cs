...
namespace myyandar.yHealth.Tests
{
    public abstract class yHealthTestBase : AbpIntegratedTestBase<yHealthTestModule>
    {
        ...
        protected void UsingDbContext(Action<yHealthDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<yHealthDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<yHealthDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<yHealthDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<yHealthDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<yHealthDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<yHealthDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<yHealthDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<yHealthDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<yHealthDbContext, Task<T>> func)
        {
            T result;
